...
vec3 SampleSkyviewLUT(vec3 rayDirection, vec3 sunDirection)
{
    ...
    const vec3 right = cross(sunDirection, up);
    const vec3 forward = cross(up, right);

    const vec3 projectedDirection = normalize(rayDirection - up * dot(rayDirection, up));
    const float sinTheta = dot(projectedDirection, right);
    const float cosTheta = dot(projectedDirection, forward);
...
void main()
{
    ...
    if (depth >= 1.f)
    {
        const float altitude = gPI / 60 - gPI / 24;
        const vec3 sunDirection = normalize(vec3(0.0, sin(altitude), -cos(altitude)));
        ...
        const vec3 color = jodieReinhardTonemap(SampleSkyviewLUT(rayDirection, sunDirection));
